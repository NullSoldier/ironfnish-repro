...
class Migrator {
    constructor(options) {
        ...
        const whitelistedDBs = options?.databases ?? utils_1.StrEnumUtils.getValues(migration_1.Database);
        this.migrations = data_1.MIGRATIONS.map((m) => {
    ...
    async migrate(options) {
        ...
        if (options?.quiet) {
            logger.level = consola_1.LogLevel.Silent;
        }
        ...
        for (const migration of unapplied) {
            ...
            try {
                ...
                if (dryRun) {
                    assert_1.Assert.isNotUndefined(tx);
                    await tx.abort();
                    break;
                }
            }
