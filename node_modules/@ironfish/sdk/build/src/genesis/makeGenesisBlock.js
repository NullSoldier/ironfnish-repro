...
exports.makeGenesisBlock = void 0;
 ...
 */
async function makeGenesisBlock(chain, info, logger) {
    ...
    const genesisKey = rust_nodejs_1.generateKey();
    // Create a genesis note granting the genesisKey allocationSum coins.
    const genesisNote = new rust_nodejs_1.Note(genesisKey.publicAddress, allocationSum, '', rust_nodejs_1.Asset.nativeId(), genesisKey.publicAddress);
    ...
    const minersFeeKey = rust_nodejs_1.generateKey();
    const note = new rust_nodejs_1.Note(minersFeeKey.publicAddress, BigInt(0), '', rust_nodejs_1.Asset.nativeId(), minersFeeKey.publicAddress);
    const minersFeeTransaction = new rust_nodejs_1.Transaction(minersFeeKey.spendingKey);
    minersFeeTransaction.output(note);
    const postedMinersFeeTransaction = new transaction_1.Transaction(minersFeeTransaction.post_miners_fee());
    /**
     *
     * Transaction 1:
    ...
    const initialTransaction = new rust_nodejs_1.Transaction(genesisKey.spendingKey);
    logger.info('  Generating the output...');
    initialTransaction.output(genesisNote);
    logger.info('  Posting the initial transaction...');
    const postedInitialTransaction = new transaction_1.Transaction(initialTransaction.post_miners_fee());
    ...
    const transaction = new rust_nodejs_1.Transaction(genesisKey.spendingKey);
    ...
    for (const alloc of info.allocations) {
        ...
        const note = new rust_nodejs_1.Note(alloc.publicAddress, BigInt(alloc.amountInOre), alloc.memo, rust_nodejs_1.Asset.nativeId(), genesisNote.owner());
    ...
    // on the block header.
    const block = await chain.newBlock(transactionList, postedMinersFeeTransaction, graffiti_1.GraffitiUtils.fromString('genesis'));
...
exports.makeGenesisBlock = makeGenesisBlock;
//# sourceMappingURL=makeGenesisBlock.js.map