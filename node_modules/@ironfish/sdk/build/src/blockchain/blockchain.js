...
exports.HeadChangedError = exports.VerifyError = exports.Blockchain = exports.VERSION_DATABASE_CHAIN = void 0;
...
const verifier_1 = require("../consensus/verifier");
const event_1 = require("../event");
const logger_1 = require("../logger");
const merkletree_1 = require("../merkletree");
const leaves_1 = require("../merkletree/database/leaves");
const nodes_1 = require("../merkletree/database/nodes");
...
const blockchaindb_1 = require("./database/blockchaindb");
const nullifierSet_1 = require("./nullifierSet/nullifierSet");
...
class Blockchain {
    constructor(options) {
        ...
        this.verifier = new verifier_1.Verifier(this, options.workerPool);
        this.addSpeed = new rollingAverage_1.RollingAverage(500);
        this.invalid = new blru_1.default(100, null, buffer_map_1.BufferMap);
        this.orphans = new blru_1.default(100, null, buffer_map_1.BufferMap);
        ...
        this.blockchainDb = new blockchaindb_1.BlockchainDB({
            files: options.files,
            location: options.location,
        });
        this.notes = new merkletree_1.MerkleTree({
            hasher: new merkletree_1.NoteHasher(),
            leafIndexKeyEncoding: storage_1.BUFFER_ENCODING,
            leafEncoding: new leaves_1.LeafEncoding(),
            nodeEncoding: new nodes_1.NodeEncoding(),
            db: this.blockchainDb.db,
            name: 'n',
            depth: 32,
            defaultValue: Buffer.alloc(32),
        });
        this.nullifiers = new nullifierSet_1.NullifierSet({ db: this.blockchainDb.db, name: 'u' });
    }
    get head() {
        assert_1.Assert.isNotNull(this._head, 'Blockchain.head should never be null. Is the chain database open?');
        return this._head;
    }
    set head(newHead) {
        this._head = newHead;
    ...
    get latest() {
        assert_1.Assert.isNotNull(this._latest, 'Blockchain.latest should never be null. Is the chain database open?');
        return this._latest;
    }
    set latest(newLatest) {
        this._latest = newLatest;
    }
    get genesis() {
        assert_1.Assert.isNotNull(this._genesis, 'Blockchain.genesis should never be null. Is the chain database open?');
        return this._genesis;
    }
    set genesis(newGenesis) {
    ...
    async seed() {
        const genesis = block_1.BlockSerde.deserialize(this.seedGenesisBlock);
        const result = await this.addBlock(genesis);
        assert_1.Assert.isTrue(result.isAdded, `Could not seed genesis: ${result.reason || 'unknown'}`);
        assert_1.Assert.isEqual(result.isFork, false);
        const genesisHeader = await this.getHeaderAtSequence(block_1.GENESIS_BLOCK_SEQUENCE);
        assert_1.Assert.isNotNull(genesisHeader, 'Added the genesis block to the chain, but could not fetch the header');
    ...
    async load() {
        let genesisHeader = await this.getHeaderAtSequence(block_1.GENESIS_BLOCK_SEQUENCE);
        if (genesisHeader) {
            assert_1.Assert.isTrue(genesisHeader.hash.equals(blockheader_1.BlockHeaderSerde.deserialize(this.seedGenesisBlock.header).hash), 'Genesis block in network definition does not match existing chain genesis block');
        }
        ...
        if (headHash) {
            ...
            assert_1.Assert.isNotNull(head, `The blockchain meta table has a head hash of ${headHash.toString('hex')}, but no block header for that hash.`);
            this.head = head;
        }
        const latestHash = await this.blockchainDb.getMetaHash('latest');
        if (latestHash) {
            const latest = await this.getHeader(latestHash);
            assert_1.Assert.isNotNull(latest, `The blockchain meta table has a latest hash of ${latestHash.toString('hex')}, but no block header for that hash.`);
            this.latest = latest;
        }
        if (this._head) {
            this.updateSynced();
    ...
    async addBlock(block) {
        ...
        try {
            connectResult = await this.blockchainDb.db.transaction(async (tx) => {
                ...
                if (!this.hasGenesisBlock && block.header.sequence === block_1.GENESIS_BLOCK_SEQUENCE) {
                    return await this.connect(block, null, tx);
                }
                const invalid = this.isInvalid(block.header);
                if (invalid) {
                    throw new VerifyError(invalid, peer_1.BAN_SCORE.MAX);
                }
                const verify = this.verifier.verifyBlockHeader(block.header);
                if (!verify.valid) {
                    assert_1.Assert.isNotUndefined(verify.reason);
                    throw new VerifyError(verify.reason, peer_1.BAN_SCORE.MAX);
                }
                if (await this.hasBlock(hash, tx)) {
                    throw new VerifyError(verifier_1.VerificationResultReason.DUPLICATE);
                }
                const previous = await this.getPrevious(block.header, tx);
                if (!previous) {
                    this.addOrphan(block.header);
                    throw new VerifyError(verifier_1.VerificationResultReason.ORPHAN);
                }
     ...
     */
    async findFork(headerA, headerB, tx) {
        if (headerA instanceof block_1.Block) {
            headerA = headerA.header;
        }
        if (headerB instanceof block_1.Block) {
            headerB = headerB.header;
        }
        let [base, fork] = headerA.sequence < headerB.sequence ? [headerA, headerB] : [headerB, headerA];
        ...
        while (!base.hash.equals(fork.hash)) {
            ...
            while (fork.sequence > base.sequence) {
                ...
                assert_1.Assert.isNotNull(prev);
                fork = prev;
            }
     ...
     */
    async *iterateToHashes(start, end, tx, reachable = true) {
        let current = start.hash;
        let last = null;
        ...
        if (reachable && end && !current?.equals(end.hash)) {
            throw new Error('Failed to iterate between blocks on diverging forks:' +
                ` curr: ${utils_1.HashUtils.renderHash(last)},` +
                ` end: ${utils_1.HashUtils.renderHash(end.hash)},` +
     ...
     * Start and end being included in the yielded blocks.
     * */
    async *iterateFrom(start, end, tx, reachable = true) {
        let current = start;
        ...
        if (reachable && end && !current?.hash.equals(end.hash)) {
            throw new Error('Failed to iterate between blocks on diverging forks:' +
                ` current: '${utils_1.HashUtils.renderHash(current?.hash)}',` +
                ` current_sequence: '${Number(current?.sequence)}',` +
                ` end: '${utils_1.HashUtils.renderHash(end.hash)}'`);
        }
    }
    isInvalid(headerOrHash) {
        ...
        if (!Buffer.isBuffer(headerOrHash) && this.invalid.has(headerOrHash.previousBlockHash)) {
            this.addInvalid(headerOrHash.hash, verifier_1.VerificationResultReason.INVALID_PARENT);
            return verifier_1.VerificationResultReason.INVALID_PARENT;
        }
        return null;
    }
    addInvalid(hash, reason) {
        this.invalid.set(hash, reason);
    }
    async connect(block, prev, tx) {
        const start = utils_1.BenchUtils.start();
        const work = block.header.target.toDifficulty();
        block.header.work = (prev ? prev.work : BigInt(0)) + work;
        let prevNoteSize = 0;
        if (prev) {
            assert_1.Assert.isNotNull(prev.noteSize);
            prevNoteSize = prev.noteSize;
        }
        block.header.noteSize = prevNoteSize + block.counts().notes;
        const isFork = !this.isEmpty && !blockheader_1.isBlockHeavier(block.header, this.head);
        if (isFork) {
        ...
        const addTime = utils_1.BenchUtils.end(start);
        ...
        if (this.logAllBlockAdd || Number(block.header.sequence) % 20 === 0) {
            this.logger.info('Added block' +
                ...
                ` hash: ${utils_1.HashUtils.renderHash(block.header.hash)},` +
    ...
    async disconnect(block, tx) {
        assert_1.Assert.isTrue(block.header.hash.equals(this.head.hash), `Cannot disconnect ${utils_1.HashUtils.renderHash(block.header.hash)} block that is not the current head ${utils_1.HashUtils.renderHash(this.head.hash)}`);
        assert_1.Assert.isFalse(block.header.sequence === block_1.GENESIS_BLOCK_SEQUENCE, 'You cannot disconnect the genesisBlock');
    ...
    async reconnect(block, tx) {
        assert_1.Assert.isTrue(block.header.previousBlockHash.equals(this.head.hash), `Reconnecting block ${block.header.hash.toString('hex')} (${block.header.sequence}) does not go on current head ${this.head.hash.toString('hex')} (${this.head.sequence - 1}) expected ${block.header.previousBlockHash.toString('hex')} (${block.header.sequence - 1})`);
        const prev = await this.getPrevious(block.header);
        assert_1.Assert.isNotNull(prev);
    ...
    async addForkToChain(block, prev, tx) {
        const verifyBlockAdd = this.verifier.verifyBlockAdd(block, prev, tx).catch((_) => {
            return { valid: false, reason: verifier_1.VerificationResultReason.ERROR };
        });
        await this.saveBlock(block, prev, true, tx);
        const { valid, reason } = await verifyBlockAdd;
        if (!valid) {
            assert_1.Assert.isNotUndefined(reason);
            this.logger.warn(`Invalid block adding to fork ${utils_1.HashUtils.renderHash(block.header.hash)} (${block.header.sequence}): ${reason}`);
            this.addInvalid(block.header.hash, reason);
            throw new VerifyError(reason, peer_1.BAN_SCORE.MAX);
        }
        await tx.update();
        this.notes.pastRootTxCommitted(tx);
        if (!this.hasGenesisBlock || blockheader_1.isBlockLater(block.header, this.latest)) {
            this.latest = block.header;
        }
        await this.onForkBlock.emitAsync(block, tx);
        this.logger.warn('Added block to fork' +
            ` seq: ${block.header.sequence},` +
            ` head-seq: ${this.head.sequence || ''},` +
            ` hash: ${utils_1.HashUtils.renderHash(block.header.hash)},` +
            ` head-hash: ${this.head.hash ? utils_1.HashUtils.renderHash(this.head.hash) : ''},` +
            ` work: ${block.header.work},` +
            ` head-work: ${this.head.work || ''},` +
            ` work-diff: ${(this.head.work || BigInt(0)) - block.header.work}`);
    }
    async addHeadToChain(block, prev, tx) {
        if (prev && !block.header.previousBlockHash.equals(this.head.hash)) {
            this.logger.warn(`Reorganizing chain from ${utils_1.HashUtils.renderHash(this.head.hash)} (${this.head.sequence}) for ${utils_1.HashUtils.renderHash(block.header.hash)} (${block.header.sequence}) on prev ${utils_1.HashUtils.renderHash(block.header.previousBlockHash)} (${block.header.sequence - 1})`);
        ...
        if (!valid) {
            assert_1.Assert.isNotUndefined(reason);
            this.logger.warn(`Invalid block adding to head chain ${utils_1.HashUtils.renderHash(block.header.hash)} (${block.header.sequence}): ${reason}`);
            this.addInvalid(block.header.hash, reason);
            throw new VerifyError(reason, peer_1.BAN_SCORE.MAX);
        }
        await tx.update();
        ...
        this.head = block.header;
        if (block.header.sequence === block_1.GENESIS_BLOCK_SEQUENCE) {
     ...
     */
    async reorganizeChain(newHead, tx) {
        const oldHead = this.head;
        assert_1.Assert.isNotNull(oldHead, 'No genesis block with fork');
        ...
        const removeHeaders = await utils_1.AsyncUtils.materialize(removeIter);
        const removeBlocks = await Promise.all(removeHeaders
        ...
        const addHeaders = await utils_1.AsyncUtils.materialize(addIter);
        const addBlocks = await Promise.all(addHeaders
            .filter((h) => !h.hash.equals(fork.hash))
            .reverse()
            .map(async (h) => {
            const block = await this.getBlock(h, tx);
        ...
        this.logger.warn('Reorganized chain.' +
            ` blocks: ${oldHead.sequence - fork.sequence + (newHead.sequence - fork.sequence)},` +
            ` old: ${utils_1.HashUtils.renderHash(oldHead.hash)} (${oldHead.sequence}),` +
            ` new: ${utils_1.HashUtils.renderHash(newHead.hash)} (${newHead.sequence}),` +
            ` fork: ${utils_1.HashUtils.renderHash(fork.hash)} (${fork.sequence})`);
        this.onReorganize.emit(oldHead, newHead, fork);
    }
    addOrphan(header) {
        this.orphans.set(header.hash, header);
    }
    resolveOrphans(block) {
     ...
     */
    async getBlock(hashOrHeader, tx) {
        const blockHeader = hashOrHeader instanceof blockheader_1.BlockHeader ? hashOrHeader : null;
        const blockHash = hashOrHeader instanceof blockheader_1.BlockHeader ? hashOrHeader.hash : hashOrHeader;
        return this.blockchainDb.db.withTransaction(tx, async (tx) => {
            ...
            if (!header || !transactions) {
                throw new Error(`DB has inconsistent state header/transaction state for ${blockHash.toString('hex')}`);
            }
            return new block_1.Block(header, transactions.transactions);
        });
    }
    /**
     * Returns true if the blockchain has a block at the given hash
     */
    async hasBlock(hash, tx) {
     ...
     */
    async hasHashesAtSequence(sequence, tx) {
     ...
     */
    async getHashesAtSequence(sequence, tx) {
     ...
     */
    async newBlock(userTransactions, minersFee, graffiti, previous, verifyBlock = true) {
        const transactions = [minersFee, ...userTransactions];
        return await this.blockchainDb.db.transaction(async (tx) => {
            const startTime = utils_1.BenchUtils.start();
            ...
            if (!this.hasGenesisBlock) {
                previousBlockHash = block_1.GENESIS_BLOCK_PREVIOUS;
                previousSequence = 0;
                target = target_1.Target.maxTarget();
                timestamp = new Date(currentTime);
            }
            else {
                const heaviestHead = this.head;
                // Sanity check that we are building on top of correct size note tree, may not be needed
                assert_1.Assert.isEqual(originalNoteSize, heaviestHead.noteSize, 'newBlock note size mismatch');
                ...
                target = target_1.Target.calculateTarget(timestamp, heaviestHead.timestamp, heaviestHead.target, this.consensus.parameters.targetBlockTimeInSeconds, this.consensus.parameters.targetBucketTimeInSeconds);
            }
            const blockNotes = [];
            ...
            const header = new blockheader_1.BlockHeader(previousSequence + 1, previousBlockHash, noteCommitment, blockheader_1.transactionCommitment(transactions), target, BigInt(0), timestamp, graffiti, noteSize, BigInt(0));
            const block = new block_1.Block(header, transactions);
            if (verifyBlock && !previousBlockHash.equals(block_1.GENESIS_BLOCK_PREVIOUS)) {
            ...
            await tx.abort();
            this.metrics.chain_newBlock.add(utils_1.BenchUtils.end(startTime));
            return block;
        });
    }
    async getHeader(hash, tx) {
        return (await this.blockchainDb.getBlockHeader(hash, tx)) || null;
    }
    async getPrevious(header, tx) {
     ...
     */
    async getHashAtSequence(sequence, tx) {
     ...
     */
    async getHeaderAtSequence(sequence) {
    ...
    async getHeadersAtSequence(sequence, tx) {
        return this.blockchainDb.getBlockHeadersAtSequence(sequence, tx);
    }
    async isHeadChain(header, tx) {
     ...
     */
    async *iterateBlockHeaders(fromHash = null, toHash = null, tx, reachable = true) {
        let from;
        if (fromHash) {
        ...
        assert_1.Assert.isNotNull(from, `Expected 'from' not to be null`);
        assert_1.Assert.isNotNull(to, `Expected 'to' not to be null`);
        for await (const header of this.iterateTo(from, to, tx, reachable)) {
            yield header;
        }
    }
    async getBlockTransactions(header, tx) {
        const block = await this.getBlock(header, tx);
        if (!block) {
            return [];
        }
        assert_1.Assert.isNotNull(header.noteSize);
    ...
    async saveConnect(block, prev, tx) {
        ...
        assert_1.Assert.isEqual(prevNotesSize, await this.notes.size(tx), 'Notes tree must match previous block header');
        ...
        for (const transaction of block.transactions) {
            ...
            await this.blockchainDb.putTransactionHashToBlockHash(transaction.hash(), block.header.hash, tx);
        }
    ...
    async saveDisconnect(block, prev, tx) {
    ...
    async saveBlock(block, prev, fork, tx) {
    ...
    async saveConnectedMintsToAssetsStore(transaction, tx) {
        for (const { asset, value } of transaction.mints) {
            ...
            await this.blockchainDb.putAsset(assetId, {
    ...
    async saveConnectedBurnsToAssetsStore(transaction, tx) {
        for (const { assetId, value } of transaction.burns) {
            const existingAsset = await this.blockchainDb.getAsset(assetId, tx);
            assert_1.Assert.isNotUndefined(existingAsset, 'Cannot burn undefined asset from the database');
            const existingSupply = existingAsset.supply;
            const supply = existingSupply - value;
            assert_1.Assert.isTrue(supply >= BigInt(0), 'Invalid burn value');
            await this.blockchainDb.putAsset(assetId, {
                createdTransactionHash: existingAsset.createdTransactionHash,
                id: existingAsset.id,
    ...
    async deleteDisconnectedMintsFromAssetsStore(transaction, tx) {
        for (const { asset, value } of transaction.mints.slice().reverse()) {
            ...
            const existingSupply = existingAsset.supply;
            const supply = existingSupply - value;
            assert_1.Assert.isTrue(supply >= BigInt(0));
    ...
    updateSynced() {
        ...
        const maxSyncedAgeMs = this.config.get('maxSyncedAgeBlocks') *
    ...
    async getAssetById(assetId, tx) {
        if (rust_nodejs_1.Asset.nativeId().equals(assetId)) {
            return {
...
exports.HeadChangedError = HeadChangedError;
