...
exports.VerificationResultReason = exports.Verifier = void 0;
...
class Verifier {
     ...
     */
    async verifyBlock(block, options = { verifyTarget: true }, tx) {
        if (serializers_1.getBlockSize(block) > this.chain.consensus.parameters.maxBlockSizeBytes) {
        ...
        const expectedTransactionCommitment = blockheader_1.transactionCommitment(block.transactions);
        ...
        const transactionHashes = new buffer_map_1.BufferSet();
        for (const [idx, tx] of block.transactions.entries()) {
            if (tx.version() !== rust_nodejs_1.TRANSACTION_VERSION) {
                return {
                    valid: false,
                    reason: VerificationResultReason.INVALID_TRANSACTION_VERSION,
                };
            }
            if (utils_1.isExpiredSequence(tx.expiration(), block.header.sequence)) {
                return {
                    valid: false,
     ...
     */
    verifyBlockHeader(blockHeader, options = { verifyTarget: true }) {
        ...
        if (blockHeader.timestamp.getTime() >
     ...
     */
    verifyBlockHeaderContextual(current, previousHeader) {
     ...
     */
    async verifyNewTransaction(transaction) {
        let verificationResult = Verifier.verifyCreatedTransaction(transaction, this.chain.consensus);
        ...
        if (transaction.version() !== rust_nodejs_1.TRANSACTION_VERSION) {
    ...
    static getMaxTransactionBytes(maxBlockSizeBytes) {
        return maxBlockSizeBytes - serializers_1.getBlockWithMinersFeeSize();
    }
    /**
     * Verify that a transaction created by the account can be accepted into the mempool
     * and rebroadcasted to the network.
     */
    static verifyCreatedTransaction(transaction, consensus) {
        if (serializers_1.getTransactionSize(transaction) >
    ...
    async verifyTransactionSpends(transaction, tx) {
    ...
    async verifyTransactionAdd(transaction, tx) {
     ...
     */
    isValidTarget(header, previous) {
        if (!this.enableVerifyTarget) {
            return true;
        }
        const expectedTarget = target_1.Target.calculateTarget(header.timestamp, previous.timestamp, previous.target, this.chain.consensus.parameters.targetBlockTimeInSeconds, this.chain.consensus.parameters.targetBucketTimeInSeconds);
        return header.target.targetValue === expectedTarget.targetValue;
    }
    // TODO: Rename to verifyBlock but merge verifyBlock into this
    async verifyBlockAdd(block, prev, tx) {
     ...
     */
    async verifyConnectedSpends(block, tx) {
        return this.chain.blockchainDb.db.withTransaction(tx, async (tx) => {
            const previousNotesSize = block.header.noteSize;
            assert_1.Assert.isNotNull(previousNotesSize);
     ...
     */
    async verifyBlockConnect(block, tx) {
     ...
     */
    async verifySpend(spend, notesSize, tx) {
     ...
     */
    async verifyConnectedBlock(block, tx) {
        return this.chain.blockchainDb.db.withTransaction(tx, async (tx) => {
    ...
    static verifyMints(mints) {
        for (const mint of mints) {
            const humanName = buffer_1.BufferUtils.toHuman(mint.asset.name());
    ...
    static verifyBurns(burns) {
        for (const burn of burns) {
            if (burn.assetId.equals(rust_nodejs_1.Asset.nativeId())) {
     ...
     */
    static verifyInternalNullifiers(spends) {
     ...
     * scenarios.
     */
    verifyUnseenTransaction(transaction, tx) {
        return this.chain.blockchainDb.db.withTransaction(tx, async (tx) => {
     ...
     */
    async verifyMintOwnersIncremental(mints, lastKnownAssetOwners, tx) {
     ...
     */
    async verifyMintOwners(mints, tx) {
...
var VerificationResultReason;
(function (VerificationResultReason) {
    VerificationResultReason["BLOCK_TOO_OLD"] = "Block timestamp is in past";
    VerificationResultReason["DESERIALIZATION"] = "Failed to deserialize";
    VerificationResultReason["DOUBLE_SPEND"] = "Double spend";
    VerificationResultReason["DUPLICATE"] = "Duplicate";
    VerificationResultReason["DUPLICATE_TRANSACTION"] = "Transaction is a duplicate";
    VerificationResultReason["ERROR"] = "Error";
    VerificationResultReason["GOSSIPED_GENESIS_BLOCK"] = "Peer gossiped its genesis block";
    VerificationResultReason["GRAFFITI"] = "Graffiti field is not 32 bytes in length";
    VerificationResultReason["HASH_NOT_MEET_TARGET"] = "Hash does not meet target";
    VerificationResultReason["INVALID_ASSET_NAME"] = "Asset name is blank";
    VerificationResultReason["INVALID_GENESIS_BLOCK"] = "Peer is using a different genesis block";
    VerificationResultReason["INVALID_MINERS_FEE"] = "Miner's fee is incorrect";
    VerificationResultReason["INVALID_MINT_OWNER"] = "Mint owner is not consistent with chain state";
    VerificationResultReason["INVALID_PARENT"] = "Invalid_parent";
    VerificationResultReason["INVALID_SPEND"] = "Invalid spend";
    VerificationResultReason["INVALID_TARGET"] = "Invalid target";
    VerificationResultReason["INVALID_TRANSACTION_COMMITMENT"] = "Transaction commitment does not match transactions";
    VerificationResultReason["INVALID_TRANSACTION_FEE"] = "Transaction fee is incorrect";
    VerificationResultReason["INVALID_TRANSACTION_PROOF"] = "Invalid transaction proof";
    VerificationResultReason["INVALID_TRANSACTION_VERSION"] = "Invalid transaction version";
    VerificationResultReason["MAX_BLOCK_SIZE_EXCEEDED"] = "Block size exceeds maximum";
    VerificationResultReason["MAX_TRANSACTION_SIZE_EXCEEDED"] = "Transaction size exceeds maximum";
    VerificationResultReason["MINERS_FEE_EXPECTED"] = "Miners fee expected";
    VerificationResultReason["MINIMUM_FEE_NOT_MET"] = "Transaction fee is below the minimum required fee";
    VerificationResultReason["NATIVE_BURN"] = "Attempting to burn the native asset";
    VerificationResultReason["NOTE_COMMITMENT"] = "Note_commitment";
    VerificationResultReason["NOTE_COMMITMENT_SIZE_TOO_LARGE"] = "Note commitment tree is smaller than referenced by the spend";
    VerificationResultReason["ORPHAN"] = "Block is an orphan";
    VerificationResultReason["PREV_HASH_MISMATCH"] = "Previous block hash does not match expected hash";
    VerificationResultReason["PREV_HASH_NULL"] = "Previous block hash is null";
    VerificationResultReason["SEQUENCE_OUT_OF_ORDER"] = "Block sequence is out of order";
    VerificationResultReason["TOO_FAR_IN_FUTURE"] = "Timestamp is in future";
    VerificationResultReason["TRANSACTION_EXPIRED"] = "Transaction expired";
    VerificationResultReason["VERIFY_TRANSACTION"] = "Verify_transaction";
})(VerificationResultReason = exports.VerificationResultReason || (exports.VerificationResultReason = {}));
