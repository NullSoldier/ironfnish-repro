...
class Strategy {
    constructor(options) {
        this.miningRewardCachedByYear = new Map();
     ...
     */
    miningReward(sequence) {
        const ironFishYearInBlocks = (365 * 24 * 60 * 60) / this.consensus.parameters.targetBlockTimeInSeconds;
        ...
        const annualReward = (this.consensus.parameters.genesisSupplyInIron / 4) * Math.E ** (-0.05 * yearsAfterLaunch);
        reward = this.convertIronToOre(utils_1.MathUtils.roundBy(annualReward / ironFishYearInBlocks, 0.125));
        this.miningRewardCachedByYear.set(yearsAfterLaunch, reward);
        return reward;
    }
    convertIronToOre(iron) {
     ...
     */
    async createMinersFee(totalTransactionFees, blockSequence, minerSpendKey) {
